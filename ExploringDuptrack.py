...
parser.add_argument("-c", "--chromosome", required=True, type=str,
                    help="chromosome in which the genetic position lies [1 through 22, X, Y, M]")
parser.add_argument("-p", "--position", required=True,  type=int,
                    help="genomic position")
...
# check whether the position provided by the user is within an annotated duplicated region
...
df.loc[(df['chromStart'] <= position) & (df['chromEnd']
                                         >= position), 'Included in Dup interval'] = 'Yes'


# 7. create and print a new dataframe containing the rows that have a Yes value in the last column


df1 = (df.loc[df['Included in Dup interval'] == 'Yes'])[['chrom', 'chromStart',
                                                         'chromEnd', 'name', 'strand', 'otherChrom', 'otherStart', 'otherEnd', 'fracMatch']]

if not df1.empty:
	print(df1)
else:
	print("The provided genomic position ", "chr" + args.chromosome,
	      position,  " does not lie within a duplicated region.")
